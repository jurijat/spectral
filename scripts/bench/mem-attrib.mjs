...
if (typeof global.gc !== 'function') { console.error('run with --expose-gc'); process.exit(2); }
const argv = process.argv.slice(2);
const DOC = argv[argv.indexOf('--doc') + 1];
const MB = b => +(b / 1048576).toFixed(1);
const settle = () => { for (let i = 0; i < 4; i++) global.gc(); return process.memoryUsage().heapUsed; };

const abs = pathResolve(process.cwd(), DOC);
const text = readFileSync(abs, 'utf8');
const srcMB = MB(Buffer.byteLength(text));
let h = settle();
const step = (label, fn) => { fn(); const n = settle(); console.log(`${label.padEnd(34)} ${String(MB(h - n)).padStart(8)} MB freed   (heapUsed now ${MB(n)} MB)`); h = n; };
const grow = (label, fn) => { const r = fn(); const n = settle(); console.log(`${label.padEnd(34)} ${String(MB(n - h)).padStart(8)} MB added   (heapUsed now ${MB(n)} MB)`); h = n; return r; };

console.log(`doc ${abs}  source ${srcMB} MB  node ${process.version}`);
console.log(`baseline heapUsed ${MB(h)} MB\n--- growth ---`);

let document = grow('parse -> Document (all)', () => new Document(text, Parsers.Yaml, abs));
const inventory = new DocumentInventory(document, new Resolver());
await grow('resolve -> inventory.resolved', async () => { await inventory.resolve(); });

console.log('\n--- destructive attribution (drop ref, force GC) ---');
const pr = document.parserResult;
step('drop inventory.resolved (2nd copy)', () => { inventory.resolved = null; });
step('drop inventory.graph', () => { inventory.graph = null; });
step('drop parserResult.ast (YAML AST)', () => { pr.ast = null; });
step('drop parserResult.lineMap', () => { pr.lineMap = null; });
step('drop parserResult.comments', () => { pr.comments = null; });
step('drop parserResult.data (unresolved)', () => { pr.data = null; });
step('drop source text', () => { document.input = null; });
